...
# === 新增：暴露出两个槽位，用来放晴天和雨天的全景贴图 ===
@export var sunny_sky_tex: Texture2D
@export var rainy_sky_tex: Texture2D
...
func _on_weather_button_pressed() -> void:
	...
	# 获取当前的天空材质
	var sky_material = env.environment.sky.sky_material as PanoramaSkyMaterial
	...
	if is_raining:
		...
		# === 核心修改：瞬间切换为阴天贴图 ===
		if sky_material and rainy_sky_tex:
			sky_material.panorama = rainy_sky_tex
		
		# 光线变暗 
		# (注意：如果你的阴天贴图本身就很黑，你可以把这里的 0.3 稍微调大一点，比如 0.6)
		...
		# === 核心修改：瞬间切换回晴天贴图 ===
		if sky_material and sunny_sky_tex:
			sky_material.panorama = sunny_sky_tex
